...
class Preprocessed_uploads extends CI_Controller
{
    ...
    public function netgen()
    {
        ...
        $sliding_window = $this->session->userdata('sliding_window');
        $cmd = '';
        // this seems to be blocking and does not create file # of individual processes
        foreach ($files as $file => $file_name) {
            ...
            $cmd = 'nice java' . ' -jar ' . $netgen_path . 'NetGenL4.jar ' . $file_path . $netgen_path . 'stopword.txt' . ' ' . $use_freq . ' ' . $freq_lower . ' ' . $freq_upper . ' '. $sliding_window . ' 2>&1 1> /dev/null & echo $!';
  ...
  //   }
    ...
    public function HistoGen()
    {
        ...
        if (!is_null($files)) {
            ...
            // exec("sort -n " . $file_path . "histo.txt | uniq -c | awk '{print $1}' | sort -n > " . $file_path . "dataX.txt");



            $files = array_map(function($value) { return $this->file_dir . '/preprocessed/'.$value; }, $files);
            // $cmd  = "cat ".implode(" ",$files)." |cut -f1 -d'/'|sort -n| uniq -c |sort -k1n |awk '{print $1\" \"$2}' > " . $file_path.'data_wordsWFreq.txt' ; 
            // exec ($cmd);
            // $cmd  = "cat ".implode(" ",$files)." |cut -f1 -d'/'|sort -n| uniq -c |sort -k1n |awk '{print $1}' |uniq -c | sort -k1nr | awk '{print $1\" \"$2}' > " . $file_path.'dataFreqBins.txt' ; 
            // exec ($cmd);
           

            $cmd  = "cat ".implode(" ",$files)." |cut -f1 -d'/'|sort -n| uniq -c |sort -k1n |awk '{print $1}' |uniq -c | sort -k1nr | awk '{print $1}' > " . $file_path.'data.txt' ; 
            exec($cmd); 
            ...
            //$cmd5 = 'rm ' . $file_path . 'data.txt';
            exec('rm ' . $file_path . 'data.txt');

			...
			<ul>
				<form id="settings" name="settings" method="post" action="settings/save_settings">
					...
					<select id="project_selector" name="project">
						...
						<option value = "<?php echo $project['project_id']; ?>" <?php if($project['project_id'] == $this->session->userdata('project_id')){ echo 'selected'; }?>><?php echo $project['name']; ?></option>
						<?php endforeach; ?>
					</select>
					<p class="current_val">Current Project: <?php if($current_project){echo $current_project->name;} ?></p>
					<button id="delete_btn" class="btn btn-danger" name="file_action" value="delete" type="submit" >Delete</button> <button class="btn btn-primary" name="file_action" value="current_project" type="submit">Save</button>
				</form>
			</ul>
